...
export default function Dashboard({ session, profile, onProfileUpdate }) {
  ...
  // NOUVEAU : État pour savoir quel bouton "Publier" est en train de charger
  ...
  const handleGenerate = async (posture) => {
    ...
    try {
      ...
      const response = await fetch(import.meta.env.VITE_N8N_WEBHOOK_URL, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'generate', // Tag pour différencier
          userId: session.user.id,
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 pb-32">
        ...
        {!preview ? (
          ...
          <div className="relative rounded-2xl overflow-hidden shadow-xl border border-gray-100 dark:border-gray-700 group">
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-4">
              ...
              <button onClick={() => { setPreview(null); setImage(null); setResults(null); }} className="bg-red-500 text-white px-4 py-2 rounded-full font-bold text-sm hover:bg-red-600 transform hover:scale-105 transition">Supprimer</button>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="flex gap-2">
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold transition whitespace-nowrap flex items-center border ${platforms[net.id] ? `${net.color} text-white border-transparent shadow-md transform scale-105` : 'bg-white dark:bg-gray-800 text-gray-400 border-gray-200 dark:border-gray-700'
                }`}
            >
              {platforms[net.id] && <span className="mr-1">✓</span>}{net.label}
            </button>
          ))}
        </div>
        ...
        {[
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div><h3 className="font-bold text-gray-900 dark:text-white text-sm">{pos.title}</h3><p className="text-xs text-gray-500">{pos.desc}</p></div>
            </div>
      ...
      {/* Résultats avec Boutons PUBLIER */}
      {results && typeof results === 'object' && (
        <div id="results-section" className="space-y-6 animate-fade-in-up pt-4 border-t border-gray-200 dark:border-gray-800">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2"><span>✍️</span> Vos brouillons</h3>
          ...
          {Object.entries(results).map(([key, val]) => (
            platforms[key] && val && (
              <div key={key} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                {/* Header de la carte */}
                <div className={`px-4 py-3 flex justify-between items-center ${key === 'linkedin' ? 'bg-[#0077b5]' : key === 'facebook' ? 'bg-[#1877f2]' : key === 'twitter' ? 'bg-black' : 'bg-gradient-to-r from-purple-500 to-pink-500'
                  }`}>
                  <span className="text-white font-bold text-sm capitalize">{key}</span>

                  {/* GROUPE DE BOUTONS : COPIER + PUBLIER */}
                  <div className="flex items-center gap-2">
                    {/* Bouton Copier */}
                    <button
                      onClick={() => handleCopy(val, key)}
                      className={`text-[10px] font-bold px-3 py-1.5 rounded-full transition flex items-center gap-1 ${copiedState[key] ? 'bg-green-500 text-white' : 'bg-white/20 text-white hover:bg-white/30'
                        }`}
                    >
                      {copiedState[key] ? '✓' : '📋'} {copiedState[key] ? 'COPIÉ' : 'COPIER'}
                    </button>

                    {/* NOUVEAU : Bouton Publier Automatique */}
                    <button
                      onClick={() => handlePublish(val, key)}
                      disabled={publishingState[key]}
                      className={`text-[10px] font-bold px-3 py-1.5 rounded-full transition flex items-center gap-1 border border-white/30 ${publishingState[key]
                          ? 'bg-white/50 cursor-not-allowed'
                          : 'bg-white text-black hover:bg-gray-100'
                        }`}
                    >
                      {publishingState[key] ? '⏳' : '🚀'} {publishingState[key] ? 'ENVOI...' : 'PUBLIER'}
                    </button>
                  </div>
                </div>
